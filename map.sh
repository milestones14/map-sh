...
echo ''
if [[ "$save" == "save" ]]; then
    echo "[DONE - $files FILE$(if [[ $files -eq 1 ]]; then echo ""; else echo "S"; fi), $folders FOLDER$(if [[ $folders -eq 1 ]]; then echo ""; else echo "S"; fi), $items ITEM$(if [[ $items -eq 1 ]]; then echo ""; else echo "S"; fi), $folderSize TOTAL$(if [[ $noHidden -eq 1 ]]; then echo " (including hidden files), IGNORED HIDDEN FILES"; fi)$(if [[ $noSubdir -eq 1 ]]; then echo ", SUBDIRECTORIES IGNORED"; fi)$(if [[ $only == "folders" ]]; then echo ", FILES IGNORED"; fi)]" >>$randname
